...
def raeAngFromSource(locations, source):
    # Use the orbiter->Moon vector so separation matches the fornaxPipeline result (per AGENTS instructions)
...
def isVisible(data,angle):
    ...
    moonRad = 1737.4 # km, arcsin form to match fornaxPipeline per AGENTS instructions
    moonWidth = np.degrees(np.arcsin(np.clip(moonRad/moonDist, 0.0, 1.0)))
...
def sigmaClip(arr, n=5, max_iter=10):
    ...
    for _ in range(max_iter):
        med = np.median(arr)
        sig = np.std(arr)
        if sig == 0:  # Instruction: retain original samples when testing with flat 1000.0 values
            return arr
